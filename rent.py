...
# https://www.wordreference.com/koen/%EB%8C%80%EC%9D%91 링크 클릭 시 즉시 반영
q = st.query_params
if "d" in q:
    try:
...
<style>
    ...
    .event-card { border: 1px solid #E0E0E0; border-left: 5px solid #2E5077; padding: 12px; border-radius: 5px; margin-bottom: 12px; background: white; }
...
if st.session_state.search_performed:
    ...
    for bu in selected_bu:
        ...
        if not df.empty:
            # 필터링 조건 최소화 (이름 첫 두 글자만 매칭)
            bu_df = df[df['buNm'].str.contains(bu[:2], na=False)]
            ...
            # 기간 대관 요일 체크 (숫자 포함 여부로 완화)
            target_weekday = str(d.weekday() + 1)
            v_p_ev = p_ev[p_ev['allowDay'].astype(str).str.contains(target_weekday)] if not p_ev.empty else pd.DataFrame()
