...
# 세션 및 파라미터 관리 (Today 버튼 및 스크롤용 유지)
...
<style>
    ...
    /* 상단 제목 박스 */
    .header-box {
        border: 1px solid #D1D9E6; border-radius: 12px; padding: 15px;
        text-align: center; background-color: #F8FAFF; margin-bottom: 10px;
    }
    ...
    /* 네비게이션 버튼 바 */
    .nav-bar {
        display: flex; border: 1px solid #D1D9E6; border-radius: 8px; 
        overflow: hidden; margin-bottom: 30px; background: white;
    }
    .nav-btn {
        flex: 1; text-align: center; padding: 10px 0; text-decoration: none;
        color: #1E3A5F; font-weight: bold; font-size: 14px; border-right: 1px solid #eee;
    }
    ...
    .bu-title {
        ...
        border-bottom: 2px solid #1E3A5F; padding-bottom: 8px; margin: 25px 0 15px 0;
    ...
    .res-badge {
        ...
        background: #FFF4E5; color: #B25E09; padding: 2px 10px;
        border-radius: 12px; font-size: 12px; font-weight: bold;
    }
    .res-place { font-size: 17px; font-weight: 800; color: #1E3A5F; margin-bottom: 8px; display: flex; align-items: center; }
    .res-info { display: flex; align-items: center; margin-bottom: 5px; font-size: 15px; color: #333; }
    .res-time { color: #FF4B4B; font-weight: bold; }
    
    /* [수정] 대관 기간 정보 명확화 */
    .res-date { font-size: 14px; color: #666; margin-top: 5px; padding-left: 28px; }
    .res-dept { font-size: 13px; color: #777; margin-top: 8px; display: flex; align-items: center; }
...
if st.session_state.search_performed:
    ...
    components.html("""<script>setTimeout(function(){window.parent.document.getElementById('result-anchor').scrollIntoView({behavior:'auto',block:'start'});},50);</script>""", height=0)
    ...
    <div class="header-box">
        <span class="header-title">성의교정 대관 현황</span>
        <span class="header-date">{d.strftime("%Y.%m.%d")}.<span class="{w_class}">({w_str})</span></span>
    </div>
    <div class="nav-bar">
        <a href="./?d={prev_d}" target="_self" class="nav-btn">◀ Before</a>
        <a href="./?d={today_d}" target="_self" class="nav-btn">Today</a>
        <a href="./?d={next_d}" target="_self" class="nav-btn">Next ▶</a>
    </div>
    """, unsafe_allow_html=True)
            ...
            if not bu_df.empty:
                ...
                if not final_df.empty:
                    ...
                    for _, row in final_df.iterrows():
                        # [가장 중요] 소제목 삭제하고 카드 내부에 기간 명시
                        is_period = row['startDt'] != row['endDt']
                        date_str = f"{row['startDt']} ~ {row['endDt']}"
                        
                        st.markdown(f"""
                        <div class="res-card">
                            <span class="res-badge">예약확정</span>
                            <div class="res-place">📍 {row['placeNm']}</div>
                            <div class="res-info">⏰ <span class="res-time">&nbsp;{row['startTime']} ~ {row['endTime']}</span></div>
                            <div class="res-info">📄 {row['eventNm']}</div>
                            {"<div class='res-date'>📅 " + date_str + "</div>" if is_period else ""}
                            <div class="res-dept">👤 {row['mgDeptNm']}</div>
                        </div>
                        """, unsafe_allow_html=True)
        if not has_content:
