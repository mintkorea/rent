...
# 2. CSS 스타일 (메인/카드는 원복, 개방일람 폰트만 확대)
st.markdown("""
<style>
    ...
    .open-room-note { font-size: 14px !important; color: #444; line-height: 1.4; background: #eee; padding: 4px 8px; border-radius: 4px; }
</style>
...
with st.form("search_form"):
    ...
    for i, bu in enumerate(ALL_BU):
        with cols[i % 2]:
            is_checked = st.checkbox(bu, value=(bu in st.session_state.selected_bus), key=f"cb_{bu}")
            if is_checked: current_selected.append(bu)
...
bg_note = "1206호 매주 금요일 10시 교육 예정 (첫 순찰 확인)" if v_wd == 5 else ("주말 지침 적용" if is_weekend else "평일 순찰 지침 준수")
