...
<style>
    ...
    .sub-label { font-size: 16px !important; font-weight: 800; color: #2E5077; display: block; margin-top: 5px; }

    /* 결과창 날짜 박스 (화살표 없이 깔끔하게) */
    .date-display-box { 
        text-align: center; background-color: #F8FAFF; padding: 15px; 
        border-radius: 12px; margin: 15px 0; border: 1px solid #D1D9E6; 
    }
    ...
    /* 카드 디자인 (성공 원본 유지) */
    .building-header { font-size: 18px !important; font-weight: bold; color: #2E5077; border-bottom: 2px solid #2E5077; padding-bottom: 5px; margin-top: 15px; }
    .section-title { font-size: 15px; font-weight: bold; color: #555; margin: 10px 0 5px 0; padding-left: 5px; border-left: 4px solid #ccc; }
    .event-card { border: 1px solid #E0E0E0; border-left: 5px solid #2E5077; padding: 10px; border-radius: 5px; margin-bottom: 8px !important; background-color: #ffffff; }
    .place-name { font-size: 15px; font-weight: bold; color: #1E3A5F; }
    .time-row { font-size: 14px; font-weight: bold; color: #FF4B4B; }
    .event-name { font-size: 13.5px; margin-top: 3px; color: #333; }
...
if st.session_state.search_performed:
    ...
    for bu in selected_bu:
                    ...
                    if not target_ev.empty:
                        ...
                        for _, row in target_ev.sort_values(by='startTime').iterrows():
                            ...
                            <div class="event-card">
                                <div style="font-size:10px; color:#888; float:right;">{s_txt}</div>
        ...
        if not has_content:
            st.markdown('<div style="color:#888; font-size:13px; padding: 5px 0;">대관 내역이 없습니다.</div>', unsafe_allow_html=True)

st.markdown('<a href="#" style="position:fixed; bottom:20px; right:20px; background:#1E3A5F; color:white; width:45px; height:45px; border-radius:50%; text-align:center; line-height:45px; text-decoration:none; font-size:12px; font-weight:bold; z-index:1000; box-shadow: 2px 2px 10px rgba(0,0,0,0.2);">TOP</a>', unsafe_allow_html=True)
