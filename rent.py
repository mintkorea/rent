...
# CSS: Streamlit의 모든 자동 여백을 0으로 덮어쓰기import streamlit as st
import requests
import pandas as pd
from datetime import datetime, date
...
# CSS: 인위적 밀착을 제거하고 간격을 1.5배 수준으로 확대
st.markdown("""
<style>
    /* 전체 레이아웃: 여백을 넉넉히 부여 */
    .block-container { 
    ...
    /* [수정] 건물명 헤더: 간격을 현재의 1.5배인 35px 수준으로 확대 */
    .building-header { 
...
selected_buildings = []
for b in ALL_B:
    if st.checkbox(b, value=(b in ["성의회관", "의생명산업연구원"]), key=f"v36_{b}"):
        selected_buildings.append(b)

st.markdown('<span class="sub-label">🗓️ 대관 유형</span>', unsafe_allow_html=True)
show_today = st.checkbox("당일 대관", value=True, key="chk_today_36")
show_period = st.checkbox("기간 대관", value=True, key="chk_period_36")

st.write(" ")
search_clicked = st.button("🔍 검색하기", use_container_width=True)

# 3. 데이터 로직 (생략 가능하나 구조 유지)
...
if search_clicked:
    ...
    for bu in selected_buildings:
        # 건물명 사이 간격을 1.5배로 넓게 배치
        st.markdown(f'<div class="building-header">🏢 {bu}</div>', unsafe_allow_html=True)
        bu_df = df[df['buNm'].str.contains(bu, na=False)].copy() if not df.empty else pd.DataFrame()
        
        if bu_df.empty:
            st.markdown('<div style="color:#888; font-size:16px; padding:15px 0;">ℹ️ 내역 없음</div>', unsafe_allow_html=True)
        else:
