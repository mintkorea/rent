...
# --- [로직 추가] 건물 선택 유지를 위한 세션 관리 ---
if 'target_date' not in st.session_state:
    st.session_state.target_date = today_kst()
if 'search_performed' not in st.session_state:
    st.session_state.search_performed = False

# URL 파라미터 처리 (날짜 및 건물 유지)
...
with st.form("search_form"):
    ...
    for i, bu in enumerate(ALL_BU):
        with cols[i % 2]:
            if st.checkbox(bu, value=(bu in st.session_state.selected_bus), key=f"f_{bu}"):
    ...
    if submit:
        ...
        st.session_state.search_performed = True
        # URL 갱신으로 필터 유지
        params = {"d": selected_date.strftime("%Y-%m-%d")}
...
if st.session_state.search_performed:
    ...
    # 네비게이션용 (필터 유지)
    bu_q = "".join([f"&b={b}" for b in st.session_state.selected_bus])
    prev_url = f"./?d={(d - timedelta(1)).strftime('%Y-%m-%d')}{bu_q}"
    next_url = f"./?d={(d + timedelta(1)).strftime('%Y-%m-%d')}{bu_q}"
    today_url = f"./?d={today_kst().strftime('%Y-%m-%d')}{bu_q}"
    ...
    if is_p_801:
        sh_note = "평일: START센터 직원 개방 / 야간 폐쇄만 수행" if not is_weekend else "주말: 학생 요청 시 해당 시간 동안만 개방"
    ...
    bg_note = "1206호 매주 금요일 10시 교육 예정" if v_wd == 5 else ("주말 지침 적용" if is_weekend else "평일 순찰 지침 준수")
