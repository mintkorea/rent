...
import requests
import pandas as pd
from datetime import datetime, dateimport streamlit as stimport streamlit as st
import requests
import pandas as pd
from datetime import datetime, date

# 1. 페이지 설정
st.set_page_config(page_title="성의교정 대관 조회", layout="centered")import streamlit as st
import requests
import pandas as pd
from datetime import datetime, date
...
<style>
    ...
    .g-badge {
        ...
        background-color: #000080 !important; /* 네이비 고정 */
...
if search_clicked:
    ...
    w_span_class = "sat-text" if w_idx == 5 else ("sun-text" if w_idx == 6 else "")
    
    # --- 조 이름 추출 (데이터에 있는 그대로 반영) ---
    group_display = "정보없음"
...
# CSS 스타일 (요일 색상 및 근무조별 배지 색상 강제 적용)
st.markdown("""
<style>
    #top-anchor { position: absolute; top: 0; left: 0; }
    .block-container { padding: 1rem 1.2rem !important; max-width: 500px !important; }
    ...
    .bg-A { background-color: #FFD700 !important; color: #000000 !important; } /* A조: 황색 + 검정글씨 */
    .bg-B { background-color: #FF0000 !important; color: #FFFFFF !important; } /* B조: 적색 + 흰색글씨 */
    .bg-C { background-color: #000080 !important; color: #FFFFFF !important; } /* C조: 네이비 + 흰색글씨 */
...
if search_clicked:
    ...
    # --- 근무조 동적 추출 로직 (KeyError 방지) ---
    group_display = "-"
    badge_bg_class = "bg-C" # 기본값 네이비
    
    if not df_raw.empty and 'groupNm' in df_raw.columns:
        valid_groups = df_raw['groupNm'].dropna()
        if not valid_groups.empty:
            raw_val = str(valid_groups.iloc[0]).strip().upper()
            # A, B, C 포함 여부에 따라 조 결정
            if 'A' in raw_val: 
                group_display, badge_bg_class = "A조", "bg-A"
            elif 'B' in raw_val: 
                group_display, badge_bg_class = "B조", "bg-B"
            else: 
                group_display, badge_bg_class = "C조", "bg-C"
    # ------------------------------------------
    ...
    if not df_raw.empty:
        ...
        for bu in selected_bu:
            ...
            # 당일 대관
            if show_today and 'startDt' in bu_df.columns:
                today_ev = bu_df[bu_df['startDt'] == bu_df.get('endDt', bu_df['startDt'])]
                if not today_ev.empty:
                    st.markdown('<div style="font-size:16px; font-weight:bold; color:#555; margin:10px 0 6px 5px; border-left:4px solid #ccc; padding-left:5px;">📌 당일 대관</div>', unsafe_allow_html=True)
                    for _, row in today_ev.iterrows():
...
<style>
    ...
    .bg-A { background-color: #FFD700 !important; color: #000000 !important; } 
    .bg-B { background-color: #FF0000 !important; color: #FFFFFF !important; } 
    .bg-C { background-color: #000080 !important; color: #FFFFFF !important; } 
...
# 3. 데이터 로직 (KeyError 방지를 위한 안전한 처리)
@st.cache_data(ttl=300)
def get_data(selected_date):
    url = "https://songeui.catholic.ac.kr/ko/service/application-for-rental_calendar.do"
    params = {"mode": "getReservedData", "start": selected_date.strftime('%Y-%m-%d'), "end": selected_date.strftime('%Y-%m-%d')}
...
if search_clicked:
    ...
    if not df_raw.empty:
        # groupNm 컬럼이 있는지 확인하고 있으면 첫번째 값 가져오기
        if 'groupNm' in df_raw.columns:
            raw_val = df_raw['groupNm'].dropna()
            if not raw_val.empty:
                val = str(raw_val.iloc[0]).strip().upper()
                g_char = val[0] if val else "C"
                group_display = f"{g_char}조"
                badge_bg_class = f"bg-{g_char}" if g_char in ['A', 'B', 'C'] else "bg-C"
    # -------------------------------
    ...
    if not df_raw.empty:
        target_weekday_num = str(w_idx + 1)
        for bu in selected_bu:
            # 부서 필터링 (KeyError 방지를 위해 필터링 전 컬럼 존재 확인)
            if 'buNm' not in df_raw.columns: continue
            bu_df = df_raw[df_raw['buNm'].str.contains(bu, na=False)].copy()
            if bu_df.empty: continue
...
if search_clicked:
    ...
    # --- 근무조 동적 추출 로직 보정 ---
    group_display = "-"
    badge_bg_class = "bg-C" # 기본값
    
    if not df_raw.empty:
        # 데이터 중 groupNm이 존재하는 첫 번째 값을 찾음
        valid_groups = df_raw[df_raw['groupNm'].notna()]['groupNm']
        if not valid_groups.empty:
            raw_group = str(valid_groups.iloc[0]).strip()
            g_char = raw_group[0].upper() if raw_group else "C"
            group_display = f"{g_char}조"
                ...
                if not today_ev.empty:
                    ...
                    for _, row in today_ev.iterrows():
                        st.markdown(f"""<div class="event-card">
                            ...
                            <div style="font-size:12px; color:#666; margin-top:5px; display:flex; justify-content:space-between;"><span>🗓️ {row['startDt']}</span><span>👥 {row['mgDeptNm']}</span></div>
                        </div>""", unsafe_allow_html=True)
                ...
                if not valid_period_ev.empty:
                    ...
                    for _, row in valid_period_ev.iterrows():
                        st.markdown(f"""<div class="event-card">
                            ...
                            <div style="font-size:15px; font-weight:bold; color:#FF4B4B; margin-top:2px;">⏰ {row['startTime']} ~ {row['endTime']}</div>
                            <div style="font-size:14px; margin-top:4px;">📄 {row['eventNm']}</div>
                            <div style="font-size:12px; color:#666; margin-top:5px; display:flex; justify-content:space-between;"><span>🗓️ {row['startDt']} ~ {row['endDt']}</span><span>👥 {row['mgDeptNm']}</span></div>
                        </div>""", unsafe_allow_html=True)
