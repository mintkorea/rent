...
<style>
    ...
    .block-container { 
        ...
        max-width: 500px !important; /* 모바일 폭에 맞춤 */
    }
    ...
    .stCheckbox label p { 
        font-size: 18px !important; /* 건물명 폰트 확대 */
        font-weight: 500;
    }
        ...
        if bu_df.empty:
            ...
            if show_today and not today_ev.empty:
                for _, row in today_ev.iterrows():
                    ...
                    st.markdown(f'<div class="event-card today-card"><span class="status-badge {s_cls}">{s_txt}</span><div style="font-weight:bold; font-size:17px; color:#1E3A5F;">📍 {row["placeNm"]} <br><span style="color:#FF4B4B;">⏰ {row["startTime"]} ~ {row["endTime"]}</span></div><div style="font-size:15px; margin-top:8px; font-weight:500;">📄 {row["eventNm"]}</div><div style="font-size:13px; color:#666; margin-top:6px;">👥 {row["mgDeptNm"]}</div></div>', unsafe_allow_html=True)
            ...
            if show_period and not period_ev.empty:
                for _, row in period_ev.iterrows():
                    s_cls = "status-y" if row['status'] == 'Y' else "status-n"
                    s_txt = "확정" if row['status'] == 'Y' else "대기"
                    st.markdown(f'<div class="event-card period-card"><span class="status-badge {s_cls}">{s_txt}</span><div style="font-weight:bold; font-size:17px; color:#1E3A5F;">📍 {row["placeNm"]} <br><span style="color:#FF4B4B;">⏰ {row["startTime"]} ~ {row["endTime"]}</span></div><div style="font-size:15px; margin-top:8px; font-weight:500;">📄 {row["eventNm"]}</div><div style="font-size:13px; color:#666; margin-top:6px;"><span style="color:#d63384; font-weight:bold;">🗓️ {row["startDt"]}~{row["endDt"]}</span><br>👥 {row["mgDeptNm"]}</div></div>', unsafe_allow_html=True)
