...
# 2. CSS 스타일 (원본 디자인 및 스크린샷 18:13 레이아웃 완벽 복구)
st.markdown("""
<style>
    ...
    .section-title { font-size: 17px; font-weight: bold; color: #333; margin: 15px 0 8px 0; }
    .event-card { border: 1px solid #E0E0E0; border-left: 5px solid #2E5077; padding: 12px; border-radius: 8px; margin-bottom: 12px; background-color: white; position: relative; box-shadow: 2px 2px 5px rgba(0,0,0,0.05); }
    ...
    .time-row { font-size: 16px; font-weight: bold; color: #FF4B4B; margin-bottom: 4px; }
    .event-name { font-size: 14px; color: #333; line-height: 1.4; font-weight: 500; }
...
selected_bu = [b for b in ALL_BUILDINGS if st.checkbox(b, value=(b in ["성의회관", "의생명산업연구원"]), key=f"v_chk_{b}")]
...
if st.session_state.search_performed:
    ...
    with c1:
        if st.button("⬅️", key="p_v_final"): change_date(-1); st.rerun()
    with c2:
    ...
    with c3:
        if st.button("➡️", key="n_v_final"): change_date(1); st.rerun()
    ...
    for bu in selected_bu:
                ...
                if not today_ev.empty:
                    ...
                    for _, row in today_ev.sort_values(by='startTime').iterrows():
                        ...
                        <div class="event-card">
                            ...
                            <div class="dept-name">👥 {row['mgDeptNm']}</div>
                        </div>
                        """, unsafe_allow_html=True)
        ...
        if not has_bu_content:
            st.markdown('<div style="color:#888; font-size:14px; padding:10px 5px;">대관 내역이 없습니다.</div>', unsafe_allow_html=True)
