...
# 2. CSS 스타일 (기존 스타일 유지 + 상시 개방 카드 스타일 추가)
st.markdown("""
<style>
    ...
    /* 추가: 강의실 개방 일람용 모바일 카드 스타일 */
    .open-card { 
        border: 1px dashed #2E5077; padding: 12px; border-radius: 8px; 
        margin-bottom: 10px; background-color: #F8FAFF; 
    }
    .open-bu-title { font-weight: bold; color: #2E5077; font-size: 14px; margin-bottom: 5px; border-bottom: 1px solid #D1D9E6; padding-bottom: 3px; }
    .open-room-item { font-size: 13px; color: #333; line-height: 1.6; }
    .open-note { font-size: 11px; color: #666; margin-top: 6px; font-style: italic; }
</style>
...
if st.session_state.search_performed:
    ...
    # 이미지 내 기간 조건 (4층: 3/2~4/30, 801호: 2/7~4/24)
    is_p_4th = (date(d.year, 3, 2) <= d <= date(d.year, 4, 30))
    is_p_801 = (date(d.year, 2, 7) <= d <= date(d.year, 4, 24))

    # 별관 정보
    bg_rooms = ["1201~1206호: 오전 개방 / 오후 폐쇄"] if not is_weekend else ["1201~1206호: 대관 현황 확인 후 개방"]
    bg_note = "1206호 매주 금요일 10시 교육 예정" if v_wd == 5 else ("주말 지침 적용" if is_weekend else "")
    
    st.markdown(f"""
    <div class="open-card">
        <div class="open-bu-title">🏢 서울성모별관</div>
        <div class="open-room-item">{"<br>".join([f"• {r}" for r in bg_rooms])}</div>
        {f'<div class="open-note">💡 {bg_note}</div>' if bg_note else ''}
    </div>
    """, unsafe_allow_html=True)

    # 성의회관 정보
    sh_rooms = []
    if not is_weekend: sh_rooms.append("421~522호: 오전 개방 (오후 원칙적 폐쇄)")
    if is_p_4th: sh_rooms.append("402~407호: 08:00 ~ 20:00 (첫/마지막 순찰)")
    if is_p_801: sh_rooms.append("801호: 09:00 ~ 21:00 (직원개방/21시폐쇄)")
    
    # 요일별 특수 개방
    if v_wd == 2: sh_rooms.append("502-1호: 19:00 ~ 22:00 (박사과정)")
    elif v_wd == 3: 
        sh_rooms.append("506호(솔로몬): 15:00경 개방")
        sh_rooms.append("502-1호: 19:00 ~ 22:00 (박사과정)")
    
    if sh_rooms:
        st.markdown(f"""
        <div class="open-card">
            <div class="open-bu-title">🏢 성의회관</div>
            <div class="open-room-item">{"<br>".join([f"• {r}" for r in sh_rooms])}</div>
            <div class="open-note">💡 학생 요청 시 무리한 퇴실 조치 금지</div>
        </div>
        """, unsafe_allow_html=True)
