...
# 1. 페이지 설정 및 디자인 복구 (image_82e95c.jpg 스타일)
st.set_page_config(page_title="성의교정 대관 조회", layout="centered")

# 요일 매핑 테이블
WEEKDAY_MAP = {"1": "월", "2": "화", "3": "수", "4": "목", "5": "금", "6": "토", "7": "일"}

def parse_allow_days(allow_day_str):
    """'1,2,3' 형태의 문자열을 '(월, 화, 수)' 형태로 변환"""
    if not allow_day_str or pd.isna(allow_day_str):
        return ""
    try:
        # 콤마로 분리하여 각 숫자에 해당하는 요일 추출
...
target_date = st.date_input("날짜선택", value=date(2026, 3, 12), label_visibility="collapsed", key="v56_date")
...
for i, b in enumerate(ALL_BUILDINGS):
    with cols[i % 2]:
        if st.checkbox(b, value=(b in ["성의회관", "의생명산업연구원"]), key=f"v56_bu_{i}"):
            selected_bu.append(b)

st.markdown('<span class="sub-label">🗓️ 대관 유형 선택</span>', unsafe_allow_html=True)
c1, c2 = st.columns(2)
with c1: show_today = st.checkbox("당일 대관 보기", value=True, key="v56_show_t")
with c2: show_period = st.checkbox("기간 대관 보기", value=True, key="v56_show_p")

st.write(" ")
search_clicked = st.button("🔍 검색하기", use_container_width=True, key="v56_search_btn")

# 3. 데이터 로직 (image_803efd.png API 활용)
...
def fetch_data(selected_date):
    ...
    try:
        ...
        # 응답 데이터에 allowDay 필드가 포함되어 있는지 확인이 필요합니다.
            ...
            if bu_df.empty:
                ...
                if show_period and not p_ev.empty:
                    ...
                    for _, row in p_ev.iterrows():
                        cls, txt = ("status-y", "예약확정") if row['status'] == 'Y' else ("status-n", "신청대기")
                        # API의 allowDay 필드 값을 직접 파싱
                        wd_info = parse_allow_days(row.get('allowDay', ''))
