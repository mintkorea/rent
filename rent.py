...
# CSS 스타일 - 원본 레이아웃 100% 복구 + 조 표시 색상만 보정
st.markdown("""
<style>
...
for b in ALL_BUILDINGS:
    if st.checkbox(b, value=(b in ["성의회관", "의생명산업연구원"]), key=f"orig_{b}"):
...
if search_clicked:
    ...
    if not df_raw.empty and 'groupNm' in df_raw.columns:
        valid_vals = df_raw['groupNm'].dropna().unique()
        if len(valid_vals) > 0:
            group_nm = str(valid_vals[0]).strip()
...
# CSS 스타일 - 원본 레이아웃 복구 및 배지 색상만 지정
st.markdown("""
<style>
