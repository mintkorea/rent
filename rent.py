...
<style>
    ...
    .date-display-box { 
        ...
        display: flex; flex-direction: column; align-items: center; /* 내부 요소 수직 중앙 정렬 */
    }
    .res-main-title { font-size: 24px !important; font-weight: 800; color: #1E3A5F; display: block; margin-bottom: 8px; }
    
    /* [핵심] 화살표가 박스 안에서 날짜와 한 세트로 묶이게 함 */
    .date-row { 
        display: flex; 
        align-items: center; 
        justify-content: center; 
        width: 100%; /* 박스 전체 너비 사용 */
        max-width: 300px; /* 너무 퍼지지 않게 제한 */
        gap: 20px;
    } 
    .nav-arrow { 
        font-size: 35px !important; /* 클릭하기 쉽게 크기 유지 */
        font-weight: bold; 
        color: #1E3A5F !important; 
        text-decoration: none !important; 
        flex-shrink: 0; /* 화살표가 찌그러지지 않게 */
        user-select: none;
    }
    .res-sub-title { font-size: 20px !important; font-weight: 700; color: #333; white-space: nowrap; }
...
if st.session_state.search_performed:
    ...
    # [수정] 박스 안에서 절대 밖으로 나가지 않도록 date-row 구조화
    st.markdown(f"""
