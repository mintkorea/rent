...
# CSS 스타일 (줄간격 최소화 + 자바스크립트 없는 자동 스크롤)import streamlit as stimport streamlit as stimport streamlit as st
import requests
import pandas as pd
from datetime import datetime, date
...
    else:
        st.info("해당 날짜에 조회된 대관 내역이 없습니다.")
새로 한 것 다 버리라니까 정말 미친다.
이 소스에서 결과값 타이틀 부분만 수정해
성의교정 대관현황
2026.03.12(목) | 근무 : C조
요일은 토 청색, 일, 공휴일 적색 반영
여기까지만 해!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
import pandas as pd
from datetime import datetime, date
import streamlit.components.v1 as components
...
    else:
        st.info("조회된 내역이 없습니다.")
이게 마지막 성공했던 건데, 버튼을 두번 누르라고 해서 하나로 만들라고 한건데 그게 안되니??????
import pandas as pd
from datetime import datetime, date
