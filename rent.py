...
# 1. 페이지 설정 (사용자님 원본 소스 유지)
st.set_page_config(page_title="성의교정 대관 조회", layout="centered")

# CSS 스타일 (사용자님 원본 소스 유지)
...
<style>
    ...
    /* 요청하신 요일 색상 */
    .sat-text { color: #0000FF !important; } /* 토요일 청색 */
    .sun-text { color: #FF0000 !important; } /* 일요일/공휴일 적색 */
...
if search_clicked:
    ...
    if not df_raw.empty and 'groupNm' in df_raw.columns:
        ...
        if len(valid_val) > 0:
            group_info = str(valid_val[0]).strip()
            if "조" not in group_info: group_info += "조"

    # 결과 타이틀 (자바스크립트 없이 순수 HTML만 사용)
    st.markdown(f"""
...
# CSS 스타일 (원본 유지 + 요일 색상 추가 / 스크립트 에러 방지를 위해 군더더기 제거)
st.markdown("""
<style>
...
<style>
    ...
    /* 요일 색상 정의 */
    .sat-text { color: #0000FF !important; } /* 토요일 청색 */
    .sun-text { color: #FF0000 !important; } /* 일요일 적색 */
...
if search_clicked:
    ...
    # 요청하신 타이틀 형식 반영 (네이비 배경 없음)
    formatted_date = target_date.strftime("%Y.%m.%d")
    st.markdown(f"""
        <div class="date-display">
            성의교정 대관현황<br>
            {formatted_date}<span class="{w_class}">({w_str})</span> | 근무 : {group_info}
        </div>
    """, unsafe_allow_html=True)
...
if search_clicked:
    ...
    # 상단 요일/조 정보 추가
    weekday_names = ["월","화","수","목","금","토","일"]
    target_weekday_name = weekday_names[target_date.weekday()]
    group_info = df_raw.iloc[0].get('groupNm', '-') if not df_raw.empty and 'groupNm' in df_raw.columns else "-"
    
    formatted_date = target_date.strftime("%Y.%m.%d")
    st.markdown(f'<div class="date-display">📋 {formatted_date} ({target_weekday_name}) | {group_info} 대관 현황</div>', unsafe_allow_html=True)
        ...
        for bu in selected_bu:
            ...
            if show_period and not period_ev.empty:
                ...
                if not valid_period_ev.empty:
                    ...
                    for _, row in valid_period_ev.iterrows():
                        s_cls, s_txt = ("status-y", "예약확정") if row['status'] == 'Y' else ("status-n", "신청대기")
                        # 이 부분에서 요일 정보를 생성하여 날짜 옆에 추가합니다.
                        day_info = get_weekday_names(row['allowDay'])
                        st.markdown(f"""
                        <div class="event-card">
                            <span class="status-badge {s_cls}">{s_txt}</span>
