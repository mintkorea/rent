...
# [날짜 변경 함수] 에러 유발 요소를 모두 제거한 순수 로직
def move_date(days):
    st.session_state.target_date += timedelta(days=days)
    st.session_state.search_performed = True

# 2. CSS 스타일 (사용자님의 디자인 박스를 완벽히 구현)
st.markdown("""
<style>
    ...
    div.stButton > button:hover { color: #FF4B4B !important; background: none !important; }

    .res-sub-title { font-size: 20px !important; font-weight: 700; color: #333; line-height: 1.6; }
...
if st.session_state.search_performed:
    ...
    # [디자인 고정] 박스 시작
    st.markdown(f'<div class="date-display-box"><span class="res-main-title">성의교정 대관 현황</span>', unsafe_allow_html=True)
    
    # 박스 안에서 컬럼을 나눠 화살표와 날짜 배치
    ...
    st.markdown('</div>', unsafe_allow_html=True) # 박스 끝
    ...
    for bu in selected_bu:
        ...
        if not found:
            st.markdown('<div style="color:#888; padding:5px;">대관 내역이 없습니다.</div>', unsafe_allow_html=True)
