...
# [기능 결합] 날짜 변경 함수 (디자인을 해치지 않고 기능만 수행)
def move_date(days):
    st.session_state.target_date += timedelta(days=days)
    st.session_state.search_performed = True

# CSS 스타일 (사용자님이 주신 디자인 그대로 유지)
st.markdown("""
<style>
...
c1, c2 = st.columns(2)
with c1: show_today = st.checkbox("당일 대관", value=True, key="t_today")
with c2: show_period = st.checkbox("기간 대관", value=True, key="t_period")
...
def get_data(d):
    ...
    try:
        res = requests.get(url, params={"mode": "getReservedData", "start": d, "end": d}, timeout=5)
        if res.status_code == 200 and res.text.strip(): return pd.DataFrame(res.json().get('res', []))
    except: pass
    return pd.DataFrame()
...
if st.session_state.search_performed:
    ...
    # [중요] 사용자님의 박스 디자인 안에 화살표 버튼을 심었습니다.
    st.markdown('<div class="date-display-box"><span class="res-main-title">성의교정 대관 현황</span>', unsafe_allow_html=True)
    col_l, col_m, col_r = st.columns([1, 3, 1])
    with col_l:
        st.button("←", key="prev_btn", on_click=move_date, args=(-1,), help="하루 전", help_type="arrow-btn")
    with col_m:
    ...
    with col_r:
        st.button("→", key="next_btn", on_click=move_date, args=(1,), help="하루 뒤", help_type="arrow-btn")
    st.markdown('</div>', unsafe_allow_html=True)

    # 결과 데이터 렌더링 (사용자님 소스 스타일)
